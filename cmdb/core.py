...
class AssetHandler(object):
    ...
    def __check_data(self,data,only_check_sn=False):
        # 判断上报来的数据是否合法,并且查询数据库里面是否含有相应的数据
        for field in data:
    ...
    def handler_asset(self):
        ...
        :return:
        """
        if self.__asset_is_new():
            self.__create_asset()
        else:
            self.__update_asset()

    def __asset_is_new(self):
        # 根据数据库里面的数据（sn或者id）是否含有对应资产类型的记录
    ...
    def data_is_available(self):
        # 判断数据是否合法
        asset_data = self.req.POST.get("asset_data")
        if asset_data:
            try:
                asset_data = json.loads(asset_data)
                self.__check_data(asset_data)
                self.clean_data = asset_data
                if not self.response['error']:
                    return True
            except ValueError as e:
                self.response_msg('error','AssetDataInvaild',str(e))
        else:
            self.response_msg('error','AssetDataInvaild','The report data is invaild')
    ...
    def _create_server_info(self,ignore_errs=False):
        try:
            ...
            self.response_msg('error','ObjectCreateException',str(e))
    ...
    def __filter_add_or_deleted_components(self, model_obj_name, data_from_db, data_source, identify_field):
        ...
        if type(data_source) is list:
            for data in data_source:                data_source_key_list.append(data.get(identify_field))
